...
Trades Orderbook::MatchOrders()
{
    ...
    // Already holding the API lock (when enabled). Use Internal to avoid deadlock.
    if (!bids_.empty())
    {
...
Orderbook::Orderbook(OrderbookConfig config)
    : config_{ config }
{
    if (config_.enableGoodForDayPrune)
...
{
    MaybeLock ordersLock{ ordersMutex_, config_.enableLocking };
